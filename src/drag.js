 ...
 * 拖拽元素有重叠吗
 */
function isOverlap(drag, drop) {
	var dragOffset = getOffset(drag),
	dropOffset = getOffset(drop),
	top = Math.max(dragOffset.top, dropOffset.top),
	right = Math.min(dragOffset.left + dragOffset.width, dropOffset.left + dropOffset.width),
	bottom = Math.min(dragOffset.top + dragOffset.height, dropOffset.top + dropOffset.height),
	left = Math.max(dragOffset.left, dropOffset.left);

	var ret = left < right && top < bottom;
	return ret;
}
